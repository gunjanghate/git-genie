...
export async function openCommandPalette(program) {
  ...
  if (selected === '__commit__') {
    const answers = await inquirer.prompt([
      ...
      { type: 'confirm', name: 'noBranch', message: 'Commit to current branch without asking?', default: true },
      { type: 'confirm', name: 'pushToMain', message: 'Push to main after commit?', default: false },
      { type: 'input', name: 'remote', message: 'Set remote origin (URL optional)', default: '', filter: v => (v||'').trim() }
    ]);
